...
function abrirFormulario(id) {
  ...
  const campos = ['titulo','descricao','atividade','area','tipo','prioridade','status','responsavel','apoio','prazo','dataconclusaoreal','entregavel','observacoes','avaliacao'];
  campos.forEach(c => {
  ...
  const aberturaEl = document.getElementById('f-abertura-display');
...
async function salvarDemanda() {
  ...
  const conclusaoVal   = document.getElementById('f-dataconclusaoreal').value;
  const pct            = parseInt(document.getElementById('f-pct').value) / 100;
  const statusAtual    = document.getElementById('f-status').value;
  const avaliacaoVal   = parseInt(document.getElementById('f-avaliacao').value) || 0;
