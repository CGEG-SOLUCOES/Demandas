...
function renderTabela(filtros) {
  ...
  tbody.innerHTML = lista.map(d => `
    <tr onclick="abrirDetalhe('${d.id}')">
      ...
      <td style="color:var(--text2);font-size:13px">${fmt(d.criadoEm)}</td>
...
function abrirFormulario(id) {
  ...
  const campos = ['titulo','descricao','atividade','area','prioridade','status','responsavel','apoio','prazo','datainicio','dataconclusaoreal','entregavel','observacoes'];
  campos.forEach(c => {
    const fieldMap = { datainicio: 'fm-datainicio' };
    const el = document.getElementById(fieldMap[c] || 'fm-' + c);
    if (!el) return;
    if ((c === 'prazo' || c === 'dataconclusaoreal' || c === 'datainicio') && d?.[c]) {
...
async function salvarDemanda() {
  ...
  try {
    ...
    if (editingId) {
      ...
      const dataInicioVal = document.getElementById('fm-datainicio')?.value;
      data.criadoEm  = dataInicioVal
        ? firebase.firestore.Timestamp.fromDate(new Date(dataInicioVal + 'T12:00:00'))
