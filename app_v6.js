...
// ── Progresso automático por status ──────────────────────
function autoProgressoPorStatus(status) {
  ...
  const mapa = {
    ...
    'Em Revisão':            25,
    'Em Andamento':          50,
    'Parcialmente Concluída': 75,
    'Concluída':             100
  };
  if (mapa[status] !== undefined) {
    if (pctEl) pctEl.value = mapa[status];
    if (pctDisplay) pctDisplay.textContent = mapa[status] + '%';
  }
}
...
function diasParaVencer(prazo, status, inicioEfetivo) {
  if (!prazo || status === 'Concluída') return '—';
  const p = prazo.toDate ? prazo.toDate() : new Date(prazo);
  // Se tem data de início efetivo, calcula prazo - inicioEfetivo
  // Senão, calcula prazo - hoje (comportamento anterior)
  let base = new Date();
  if (inicioEfetivo) {
    base = inicioEfetivo.toDate ? inicioEfetivo.toDate() : new Date(inicioEfetivo);
  }
  const diff = Math.floor((p - base) / (1000 * 60 * 60 * 24));
...
function badgeStatus(s) {
  const map = {
    'Concluída':             'concluida',
    'Parcialmente Concluída':'parcial',
...
function navigate(pageId) {
  ...
  if (pageId === 'demandas') {
    ...
    const fr = document.getElementById('f-responsavel'); if(fr) fr.selectedIndex = 0;
  ...
  if (pageId === 'projetos-hub') {} // cards estáticos
  if (pageId === 'calendario') renderCalendario();
  if (pageId === 'reuniao')    montarPreview();
...
function loadDemandas() {
  db.collection('demandas')
    ...
    .onSnapshot(snap => {
      ...
      if (activePage === 'equipe')    renderEquipe();
      if (activePage === 'relatorios') renderRelatorios();
      if (activePage === 'reuniao')   montarPreview();
    }, err => {
  ...
  el.innerHTML = lista.map(d => `
    <div class="recente-item" onclick="abrirDetalhe('${d.id}')">
      ...
      <div class="recente-info">
        ...
        <div class="recente-meta">${d.responsavel || '—'} · Prazo: ${fmt(d.prazo)} · ${diasParaVencer(d.prazo, d.status, d.inicioEfetivo)}</div>
      </div>
...
function renderTabela(filtros) {
  ...
  if (filtros.responsavel)  lista = lista.filter(d => normalizar(d.responsavel).includes(normalizar(filtros.responsavel)));
  ...
  if (ordenar === 'prazo') {
    lista.sort((a,b) => {
      const ap = a.prazo?.toDate ? a.prazo.toDate() : new Date(a.prazo || '9999');
      const bp = b.prazo?.toDate ? b.prazo.toDate() : new Date(b.prazo || '9999');
      return ap - bp;
    });
  } else if (ordenar === 'prazo-desc') {
    lista.sort((a,b) => {
      const ap = a.prazo?.toDate ? a.prazo.toDate() : new Date(a.prazo || '0');
      const bp = b.prazo?.toDate ? b.prazo.toDate() : new Date(b.prazo || '0');
      return bp - ap;
    ...
    lista.sort((a,b) => (ord[a.prioridade]??4) - (ord[b.prioridade]??4));
  } else if (ordenar === 'abertura') {
    lista.sort((a,b) => {
      const at = a.criadoEm?.toDate ? a.criadoEm.toDate() : new Date(a.criadoEm || '0');
      const bt = b.criadoEm?.toDate ? b.criadoEm.toDate() : new Date(b.criadoEm || '0');
      return bt - at;
    });
  ...
  tbody.innerHTML = lista.map(d => `
    <tr onclick="abrirDetalhe('${d.id}')">
      ...
      <td style="color:var(--text2);font-size:13px">${fmt(d.inicioEfetivo || d.criadoEm)}</td>
      <td style="color:var(--text2);font-size:13px">${fmt(d.prazo)}</td>
      <td style="font-size:12px">${diasParaVencer(d.prazo, d.status, d.inicioEfetivo)}</td>
    </tr>
  `).join('');
}
...
function renderMinhas() {
  ...
  // Ordenação
  const ordenarMinhas = document.getElementById('minhas-ordenar')?.value || '';
  if (ordenarMinhas === 'prazo') {
    minhas.sort((a,b) => {
      const ap = a.prazo?.toDate ? a.prazo.toDate() : new Date(a.prazo || '9999');
      const bp = b.prazo?.toDate ? b.prazo.toDate() : new Date(b.prazo || '9999');
      return ap - bp;
    });
  } else if (ordenarMinhas === 'prazo-desc') {
    minhas.sort((a,b) => {
      const ap = a.prazo?.toDate ? a.prazo.toDate() : new Date(a.prazo || '0');
      const bp = b.prazo?.toDate ? b.prazo.toDate() : new Date(b.prazo || '0');
      return bp - ap;
    });
  } else if (ordenarMinhas === 'prioridade') {
    const ord = { 'Urgente':0, 'Alta':1, 'Média':2, 'Baixa':3 };
    minhas.sort((a,b) => (ord[a.prioridade]??4) - (ord[b.prioridade]??4));
  }
...
function abrirFormulario(id) {
  ...
  const campos = ['titulo','descricao','atividade','area','prioridade','status','responsavel','apoio','prazo','inicioefetivodatainicio','dataconclusaoreal','entregavel','observacoes'];
  campos.forEach(c => {
    const fieldMap = { inicioefetivodatainicio: 'fm-inicioefetivodatainicio' };
    const el = document.getElementById(fieldMap[c] || 'fm-' + c);
    if (!el) return;
    if ((c === 'prazo' || c === 'dataconclusaoreal' || c === 'inicioefetivodatainicio') && d?.[c]) {
  ...
  // Preenche Data de Início Efetivo
  const inicioEfetivoEl = document.getElementById('fm-inicioefetivodatainicio');
  if (inicioEfetivoEl && d?.inicioEfetivo) {
    const dt = d.inicioEfetivo.toDate ? d.inicioEfetivo.toDate() : new Date(d.inicioEfetivo);
    inicioEfetivoEl.value = dt.toISOString().split('T')[0];
  } else if (inicioEfetivoEl) {
    inicioEfetivoEl.value = '';
  }
...
async function salvarDemanda() {
  ...
  const data = {
    ...
    inicioEfetivo:      (() => {
      const v = document.getElementById('fm-inicioefetivodatainicio')?.value;
      return v ? firebase.firestore.Timestamp.fromDate(new Date(v + 'T12:00:00')) : null;
    })(),
  ...
  try {
    ...
    if (editingId) {
      ...
      const dataInicioEfetivoVal = document.getElementById('fm-inicioefetivodatainicio')?.value;
      if (dataInicioEfetivoVal) {
        data.inicioEfetivo = firebase.firestore.Timestamp.fromDate(new Date(dataInicioEfetivoVal + 'T12:00:00'));
      }
...
function abrirDetalhe(id) {
  ...
  document.getElementById('det-abertura').textContent    = d.criadoEm ? fmt(d.criadoEm) : '—';
  const detInicioEl = document.getElementById('det-inicioefetivodatainicio');
  if (detInicioEl) detInicioEl.textContent = d.inicioEfetivo ? fmt(d.inicioEfetivo) : '—';
  ...
  document.getElementById('det-diasvencer').innerHTML    = diasParaVencer(d.prazo, d.status, d.inicioEfetivo);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  ['r-filtro-resp','r-filtro-status','r-filtro-area','r-filtro-prio'].forEach(id => {
    document.getElementById(id)?.addEventListener('change', () => montarPreview());
  });
  document.getElementById('f-area')?.addEventListener('change', () => renderTabela());
  document.getElementById('f-responsavel')?.addEventListener('change', () => renderTabela());
...
function limparFicha() {
  const ids = ['pj-nome','pj-patrocinador','pj-setor-sol','pj-nome-sol','pj-area-executora',
    'pj-alinhamento','pj-data-sol','pj-como-chegou',
    'pj-necessidade','pj-descricao',
    'pj-setor-dest','pj-publico',
    'pj-inicio','pj-termino',
    'pj-premissas','pj-restricoes','pj-dependencias',
    'pj-indicadores','pj-beneficios','pj-criterios-aceite',
    'pj-gestor','pj-coordenadora','pj-responsavel'];
  ids.forEach(id => {
...
async function gerarFichaWord() {
    ...
    sections: [{
      ...
      children: [
        ...
        new Paragraph({
          ...
          children: [new TextRun({ text: 'TERMO DE ABERTURA DE PROJETO', bold: true, size: 28, font: 'Arial', color: corPrimaria })]
        }),
        ...
        tabelaSecao([
          ...
          labelValor('Patrocinador', pj('pj-patrocinador')),
          labelValor('Setor solicitante', pj('pj-setor-sol')),
          labelValor('Nome do solicitante', pj('pj-nome-sol')),
          labelValor('Área executora', pj('pj-area-executora')),
          labelValor('Alinhamento estratégico', pj('pj-alinhamento')),
          labelValor('Data da solicitação', fmtData(document.getElementById('pj-data-sol')?.value)),
          labelValor('Origem da demanda', pj('pj-como-chegou')),
        ]),
        espacamento(),

        // 2. Contexto e Justificativa
        titulo('2. Contexto e Justificativa'),
        tabelaSecao([
          labelValor('Necessidade do negócio', pj('pj-necessidade')),
          labelValor('Descrição do Projeto', pj('pj-descricao')),
        ]),
        espacamento(),
        ...
        titulo('5. Premissas e Restrições'),
        tabelaSecao([
          labelValor('Premissas', pj('pj-premissas')),
          labelValor('Restrições', pj('pj-restricoes')),
          labelValor('Dependências', pj('pj-dependencias')),
        ]),
        espacamento(),

        // 6. Critérios de Sucesso
        titulo('6. Critérios de Sucesso'),
        tabelaSecao([
          labelValor('Indicadores', pj('pj-indicadores')),
          labelValor('Benefícios esperados', pj('pj-beneficios')),
          labelValor('Critérios de aceite', pj('pj-criterios-aceite')),
        ]),
        espacamento(),
              ...
              children: [
                new TableCell({
                  ...
                  children: [
                    ...
                    new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: pj('pj-gestor'), size: 18, font: 'Arial', color: '555555' })] }),
                    new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 80 }, children: [new TextRun({ text: 'Data: ___/___/______', size: 18, font: 'Arial', color: '888888' })] }),
                  ]
                }),
                new TableCell({
                  borders,
                  ...
                  children: [
                    ...
                    new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: pj('pj-coordenadora'), size: 18, font: 'Arial', color: '555555' })] }),
                    new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 80 }, children: [new TextRun({ text: 'Data: ___/___/______', size: 18, font: 'Arial', color: '888888' })] }),
                  ]
                })
              ]
            ...
            new TableRow({
              children: [
                new TableCell({
                  ...
                  children: [
                    new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: '_______________________________', size: 20, font: 'Arial' })] }),
                    new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 80 }, children: [new TextRun({ text: 'Responsável do Projeto', bold: true, size: 20, font: 'Arial' })] }),
                    new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: pj('pj-responsavel'), size: 18, font: 'Arial', color: '555555' })] }),
                    new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 80 }, children: [new TextRun({ text: 'Data: ___/___/______', size: 18, font: 'Arial', color: '888888' })] }),
                  ]
                })
              ]
  ...
  a.href = url;
  a.download = `TAP_${nomeArq}_${new Date().toISOString().slice(0,10)}.docx`;
  a.click();
  URL.revokeObjectURL(url);
...
function renderCalendario() {
  ...
  for (let dia = 1; dia <= ultimoDia; dia++) {
    ...
    // Número do dia — ANTES do onclick para não apagar o evento
    diaDiv.innerHTML = `
    ...
    if (demandas.length) {
      ...
      const _dia = dia;
      const _dem = demandas.slice();
      diaDiv.onclick = function() { abrirCalModal(_dia, _dem); };
    }
...
function montarPreview() {
  const pauta = filtrarPauta();
  const el = document.getElementById('r-preview');
  if (el) el.textContent = pauta.length
    ? `${pauta.length} demanda${pauta.length !== 1 ? 's' : ''} na pauta — ordenadas por prioridade e atraso`
    : 'Nenhuma demanda encontrada com esses filtros';
}

function filtrarPauta() {
  const resp   = document.getElementById('r-filtro-resp')?.value || '';
  const status = document.getElementById('r-filtro-status')?.value || '';
  const area   = document.getElementById('r-filtro-area')?.value || '';
  const prio   = document.getElementById('r-filtro-prio')?.value || '';

  let lista = allDemandas.filter(d => d.status !== 'Concluída');
  ...
  if (resp)   lista = lista.filter(d => normalizar(d.responsavel).includes(normalizar(resp)));
  if (area)   lista = lista.filter(d => d.area === area);
  if (prio)   lista = lista.filter(d => d.prioridade === prio);

  // Ordena: atrasadas > urgente > alta > média > baixa
  const ordemPrio = { 'Urgente': 0, 'Alta': 1, 'Média': 2, 'Baixa': 3 };
  lista.sort((a, b) => {
    const aAt = isAtrasada(a) ? -1 : 0;
    const bAt = isAtrasada(b) ? -1 : 0;
    if (aAt !== bAt) return aAt - bAt;
    return (ordemPrio[a.prioridade] ?? 4) - (ordemPrio[b.prioridade] ?? 4);
  });
...
function iniciarReuniao() {
  pautaReuniao = filtrarPauta();
  if (!pautaReuniao.length) { toast('Nenhuma demanda encontrada com esses filtros!', 'error'); return; }
  pautaIndex   = 0;
  pautaNotas   = {};
  pautaUpdates = {};

  document.getElementById('reuniao-config').style.display = 'none';
  document.getElementById('reuniao-ativa').style.display  = 'block';

  const mes = new Intl.DateTimeFormat('pt-BR', { month: 'long', year: 'numeric' }).format(new Date());
  document.getElementById('r-mes-label').textContent = `Reunião mensal — ${mes}`;

  renderCardReuniao();
}

function sairReuniao() {
  if (!confirm('Sair do modo reunião? As notas não salvas serão perdidas.')) return;
  document.getElementById('reuniao-config').style.display = 'block';
  document.getElementById('reuniao-ativa').style.display  = 'none';
}

function navReuniao(delta) {
  // Salva nota atual antes de navegar
  salvarNotaAtual();

  pautaIndex += delta;
  if (pautaIndex < 0) pautaIndex = 0;
  if (pautaIndex >= pautaReuniao.length) pautaIndex = pautaReuniao.length - 1;
  renderCardReuniao();
}

function salvarNotaAtual() {
  const nota = document.getElementById('r-nota-input')?.value;
  const novoStatus = document.getElementById('r-novo-status')?.value;
  const novoPct = document.getElementById('r-novo-pct')?.value;
  const d = pautaReuniao[pautaIndex];
  if (!d) return;
  if (nota) pautaNotas[d.id] = nota;
  if (novoStatus || novoPct) {
    pautaUpdates[d.id] = { status: novoStatus, pct: novoPct };
  }
}
...
async function salvarEAvancar() {
  salvarNotaAtual();
  const d = pautaReuniao[pautaIndex];
  const upd = pautaUpdates[d.id];

  if (upd && (upd.status || upd.pct)) {
    try {
      const data = {};
      if (upd.status) data.status = upd.status;
      if (upd.pct !== undefined) data.porcentoConcluido = parseInt(upd.pct) / 100;
      data.atualizadoEm  = firebase.firestore.FieldValue.serverTimestamp();
      data.atualizadoPor = currentUser?.email;
      await db.collection('demandas').doc(d.id).update(data);
  ...
  if (pautaIndex < pautaReuniao.length - 1) {
    navReuniao(1);
  } else {
    toast('Fim da pauta! Reunião concluída.');
  }
}

function aplicarFiltroLive() {
  // Filtra a lista sem reiniciar a reunião
  const resp   = document.getElementById('r-live-resp')?.value || '';
  const status = document.getElementById('r-live-status')?.value || '';
  const area   = document.getElementById('r-live-area')?.value || '';
  const prio   = document.getElementById('r-live-prio')?.value || '';

  let lista = [...pautaReuniao];
  if (resp)   lista = lista.filter(d => normalizar(d.responsavel).includes(normalizar(resp)));
  if (status) lista = lista.filter(d => d.status === status);
  if (area)   lista = lista.filter(d => d.area === area);
  if (prio)   lista = lista.filter(d => d.prioridade === prio);

  document.getElementById('r-total-badge').textContent = lista.length + ' demandas';
  renderListaPauta(lista);
}

function renderCardReuniao() {
  const total = pautaReuniao.length;
  const atual = pautaIndex + 1;
  const pct   = Math.round(pautaIndex / total * 100);

  document.getElementById('r-prog-txt').textContent  = `Demanda ${atual} de ${total}`;
  document.getElementById('r-prog-fill').style.width = pct + '%';
  document.getElementById('r-total-badge').textContent = total + ' demandas';

  const d = pautaReuniao[pautaIndex];
  if (!d) return;

  const atrasada = isAtrasada(d);
  const corPrio  = { Urgente: 'var(--red)', Alta: 'var(--amber)', 'Média': 'var(--accent)', Baixa: 'var(--text3)' };
  const pctAtual = Math.round((d.porcentoConcluido || 0) * 100);
  const notaSalva = pautaNotas[d.id] || '';
  const updSalvo  = pautaUpdates[d.id] || {};

  document.getElementById('r-card-demanda').innerHTML = `
    <div style="font-size:12px;color:#a0a4b0;margin-bottom:8px">Demanda ${atual} de ${total}</div>
    <div style="font-size:18px;font-weight:600;color:#fff;margin-bottom:4px">${d.titulo || '—'}</div>
    <div style="font-size:13px;color:#a0a4b0;margin-bottom:14px">${d.atividade || ''}</div>

    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px">
      ${atrasada ? '<span class="badge badge-urgente">Atrasada</span>' : ''}
      ${badgeStatus(d.status)}
      ${badgePrioridade(d.prioridade)}
      ${d.area ? `<span class="badge badge-pausada">${d.area}</span>` : ''}
    </div>

    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px">
      <div style="background:var(--bg3);border-radius:var(--radius-sm);padding:10px">
        <div style="font-size:11px;color:#555b6e;text-transform:uppercase;letter-spacing:0.04em;margin-bottom:3px">Responsável</div>
        <div style="font-size:14px;font-weight:500;color:#e0e2ea">${d.responsavel || '—'}</div>
      </div>
      <div style="background:var(--bg3);border-radius:var(--radius-sm);padding:10px">
        <div style="font-size:11px;color:#555b6e;text-transform:uppercase;letter-spacing:0.04em;margin-bottom:3px">Apoio</div>
        <div style="font-size:14px;font-weight:500;color:#e0e2ea">${d.apoio || '—'}</div>
      </div>
      <div style="background:var(--bg3);border-radius:var(--radius-sm);padding:10px">
        <div style="font-size:11px;color:#555b6e;text-transform:uppercase;letter-spacing:0.04em;margin-bottom:3px">Prazo</div>
        <div style="font-size:14px;font-weight:500;color:${atrasada ? 'var(--red)' : '#e0e2ea'}">${fmt(d.prazo)}</div>
      </div>
      <div style="background:var(--bg3);border-radius:var(--radius-sm);padding:10px">
        <div style="font-size:11px;color:#555b6e;text-transform:uppercase;letter-spacing:0.04em;margin-bottom:3px">Dias em aberto</div>
        <div style="font-size:14px;font-weight:500;color:#e0e2ea">${diasEmAberto(d.criadoEm)}</div>
      </div>
    </div>

    <div style="margin-bottom:14px">
      <div style="display:flex;justify-content:space-between;font-size:12px;color:#a0a4b0;margin-bottom:5px">
        <span>Progresso atual</span><span>${pctAtual}%</span>
      </div>
      <div style="height:8px;background:var(--bg4);border-radius:99px;overflow:hidden">
        <div style="width:${pctAtual}%;height:100%;background:var(--accent);border-radius:99px"></div>
      </div>
    </div>

    <div style="background:var(--bg3);border-radius:var(--radius-sm);padding:14px;margin-bottom:14px">
      <div style="font-size:12px;color:#a0a4b0;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:10px">Atualizar na reunião</div>
      <div style="margin-bottom:10px">
        <div style="font-size:12px;color:#a0a4b0;margin-bottom:6px">Novo status</div>
        <select class="form-select" id="r-novo-status" style="width:100%">
          <option value="">Manter atual (${d.status})</option>
          <option value="Em Andamento" ${updSalvo.status==='Em Andamento'?'selected':''}>Em Andamento</option>
          <option value="Em Revisão" ${updSalvo.status==='Em Revisão'?'selected':''}>Em Revisão</option>
          <option value="Pausada" ${updSalvo.status==='Pausada'?'selected':''}>Pausada</option>
          <option value="Concluída" ${updSalvo.status==='Concluída'?'selected':''}>Concluída</option>
        </select>
      </div>
      <div>
        <div style="display:flex;justify-content:space-between;font-size:12px;color:#a0a4b0;margin-bottom:6px">
          <span>Novo progresso</span>
          <span id="r-pct-display">${updSalvo.pct ?? pctAtual}%</span>
        </div>
        <input type="range" id="r-novo-pct" min="0" max="100" value="${updSalvo.pct ?? pctAtual}"
          style="width:100%;accent-color:var(--accent)"
          oninput="document.getElementById('r-pct-display').textContent=this.value+'%'" />
      </div>
    </div>

    <div style="margin-bottom:14px">
      <div style="font-size:12px;color:#a0a4b0;margin-bottom:6px">Nota da reunião</div>
      <textarea id="r-nota-input" class="form-textarea" placeholder="Registre o que foi discutido, decisões, próximos passos...">${notaSalva}</textarea>
    </div>

    <div style="display:flex;justify-content:space-between;align-items:center;padding-top:14px;border-top:1px solid var(--border)">
      <span style="font-size:12px;color:#a0a4b0">Atualizado por: ${d.atualizadoPor || d.criadoPor || '—'}</span>
      <div style="display:flex;gap:8px">
        <button class="btn btn-ghost btn-sm" onclick="abrirDetalhe('${d.id}')">Ver completo</button>
        <button class="btn btn-primary btn-sm" onclick="salvarEAvancar()">
          ${pautaIndex < pautaReuniao.length - 1 ? 'Salvar e avançar →' : 'Salvar e concluir ✓'}
        </button>
      </div>
    </div>
  `;

  renderListaPauta(pautaReuniao);
  renderResumo();
}

function renderListaPauta(lista) {
  const el = document.getElementById('r-lista-pauta');
  if (!el) return;
  el.innerHTML = lista.map((d, i) => {
    const isAtual = d.id === pautaReuniao[pautaIndex]?.id;
    const temNota = !!pautaNotas[d.id];
    const atrasada = isAtrasada(d);
    const cor = atrasada ? 'var(--red)' : d.prioridade === 'Urgente' ? 'var(--red)' : d.prioridade === 'Alta' ? 'var(--amber)' : 'var(--accent)';
    return `
      <div style="display:flex;align-items:center;gap:8px;padding:7px 0;border-bottom:1px solid var(--border);cursor:pointer"
           onclick="pautaIndex=${pautaReuniao.indexOf(d)};renderCardReuniao()">
        <div style="width:20px;height:20px;border-radius:50%;background:${isAtual ? 'var(--accent)' : 'var(--bg4)'};
                    font-size:11px;display:flex;align-items:center;justify-content:center;
                    color:${isAtual ? '#fff' : '#a0a4b0'};flex-shrink:0">${pautaReuniao.indexOf(d)+1}</div>
        <div style="width:7px;height:7px;border-radius:50%;background:${cor};flex-shrink:0"></div>
        <div style="flex:1;min-width:0">
          <div style="font-size:12px;color:${isAtual ? '#fff' : '#c0c4d0'};font-weight:${isAtual?'500':'400'};
                      white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${d.titulo || '—'}</div>
          <div style="font-size:11px;color:#a0a4b0">${d.responsavel || '—'}</div>
        </div>
        ${temNota ? '<span style="font-size:10px;color:var(--green)">✓</span>' : ''}
      </div>`;
  }).join('');
}

function renderResumo() {
  const el = document.getElementById('r-resumo');
  if (!el) return;
  const total      = pautaReuniao.length;
  const discutidas = pautaIndex;
  const atualizadas = Object.keys(pautaUpdates).length;
  const atrasadas  = pautaReuniao.filter(isAtrasada).length;
  el.innerHTML = `
    <div style="background:var(--bg3);border-radius:var(--radius-sm);padding:8px;text-align:center">
      <div style="font-size:11px;color:#a0a4b0;margin-bottom:3px">Discutidas</div>
      <div style="font-size:18px;font-weight:600;color:var(--green)">${discutidas}</div>
    </div>
    <div style="background:var(--bg3);border-radius:var(--radius-sm);padding:8px;text-align:center">
      <div style="font-size:11px;color:#a0a4b0;margin-bottom:3px">Restantes</div>
      <div style="font-size:18px;font-weight:600;color:#e0e2ea">${total - discutidas}</div>
    </div>
    <div style="background:var(--bg3);border-radius:var(--radius-sm);padding:8px;text-align:center">
      <div style="font-size:11px;color:#a0a4b0;margin-bottom:3px">Atualizadas</div>
      <div style="font-size:18px;font-weight:600;color:var(--accent)">${atualizadas}</div>
    </div>
    <div style="background:var(--bg3);border-radius:var(--radius-sm);padding:8px;text-align:center">
      <div style="font-size:11px;color:#a0a4b0;margin-bottom:3px">Atrasadas</div>
      <div style="font-size:18px;font-weight:600;color:var(--red)">${atrasadas}</div>
    </div>`;
}
