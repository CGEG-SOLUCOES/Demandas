...
function renderCalendario() {
  ...
  // Guarda no window para acesso global
  window._calDemandas = demandaPorDia;
  ...
  for (let i = 0; i < primeiroDia; i++) {
    const el = document.createElement('div');
    el.style.cssText = 'min-height:100px;border-right:1px solid var(--border);border-bottom:1px solid var(--border);background:var(--bg3);opacity:0.4';
    grid.appendChild(el);
  }
  ...
  for (let dia = 1; dia <= ultimoDia; dia++) {
    ...
    diaDiv.style.cssText = `min-height:100px;border-right:1px solid var(--border);border-bottom:1px solid var(--border);padding:8px;position:relative;background:${isHoje ? 'rgba(62,207,142,0.06)' : 'transparent'};${isHoje ? 'box-shadow:inset 0 0 0 1px rgba(62,207,142,0.3);' : ''}${demandas.length ? 'cursor:pointer;' : ''}`;

    // Número do dia
    const numDiv = document.createElement('div');
    numDiv.style.cssText = `font-size:13px;font-weight:${isHoje?'700':'500'};color:${isHoje?'var(--green)':'#c0c4d0'};margin-bottom:6px`;
    ...
    demandas.slice(0,3).forEach(d => {
      ...
      item.style.cssText = `font-size:11px;padding:2px 6px;border-radius:4px;margin-bottom:3px;background:${cor}22;border-left:2px solid ${cor};color:${d.status==='Concluída'?'var(--text3)':'#e0e2ea'};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;${d.status==='Concluída'?'text-decoration:line-through;':''}`;
    ...
    if (demandas.length > 3) {
      ...
      mais.style.cssText = 'font-size:11px;color:var(--text3);margin-top:2px';
    ...
    // Hover
    if (demandas.length) {
      diaDiv.addEventListener('mouseenter', () => diaDiv.style.background = 'var(--bg3)');
      diaDiv.addEventListener('mouseleave', () => diaDiv.style.background = isHoje ? 'rgba(62,207,142,0.06)' : 'transparent');
      
      // Click — usa closure direta com o valor de dia
      ;(function(d, demandasDia) {
        diaDiv.addEventListener('click', function() {
          abrirCalModal(d, demandasDia);
        });
      })(dia, demandas);
    }
